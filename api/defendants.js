...
async function loadDefendantDetail(canonicalId, { caseLimit = 100, clientLimit = 100 } = {}) {
  ...
  // Clients whose collectionsHistory names this defendant — scan the index.
  // The match index doesn't store defendant→clients directly. We walk the
  // client list and filter; capped to clientLimit for response budget.
  ...
  return {
    ...
    linkedClients,
    linkedClientCapped: linkedClients.length >= clientLimit,
  };
}
